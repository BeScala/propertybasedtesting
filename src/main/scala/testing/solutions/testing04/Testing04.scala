package testing.solutions.testing04
...
case class Prop(check: (NumberOfTestCases, RNG) => Result) {
  ...
  def &&(thatProp: Prop) = Prop {
    (numberOfTestCases, rng) => thisProp.check(numberOfTestCases, rng) match {
      case Passed => thatProp.check(numberOfTestCases, rng)
      case result => result
    }
  }
  ...
  def ||(thatProp: Prop) = Prop {
    (numberOfTestCases, rng) => thisProp.check(numberOfTestCases, rng) match {
      case Falsified(_, _) => thatProp.check(numberOfTestCases, rng)
      case result => result
    }
  }
...
object Testing04 extends App {
  ...
  private def check(prop: Prop): Unit = {
    ...
    println(prop.check(numberOfTestCases, Simple(System.nanoTime())) match {
      case Passed =>
