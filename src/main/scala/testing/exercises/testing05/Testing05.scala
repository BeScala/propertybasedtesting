package testing.exercises.testing05
...
object SGen {
  ...
  def nonEmptyList[A](ga: Gen[A]): SGen[List[A]] = ??? // Hint: simply (brute force) replace size 0 by size 1

  val double: SGen[Double] = ???
...
object Testing05 extends App {
  ...
  val sortedListIsSortedOK = { (list: List[Int]) =>
    ...
    list.isEmpty || list.tail.isEmpty || !sortedList.zip(sortedList.tail).exists { case (a, b) => a > b }
  }
